...
export default function VirtualKeyboard(props) {
  ...
  const virtualKeyDownHandler = (e) => {
    ...
    if (keyPressed) {
      ...
      elementPressed.addEventListener(
        'pointerleave',
        () => {
  ...
  useEffect(() => {
    ...
    let virtualKeys =
      virtualKeyboard.current?.querySelectorAll('.keyboard-note');
    virtualKeys.forEach((virtualKey) => {
      // Add Pointer Events
      virtualKey.addEventListener('pointerdown', virtualKeyDownHandler);
    });
  }, [virtualKeyboard, props.octave, virtualKeyDownHandler]);
    ...
    <>
      <section
        className={`virtual-keyboard ${
          props.touchControls && props.inputType === 'virtualKeyboard'
            ? 'touch-enabled'
            : ''
        } ${props.inputType === 'midi' ? 'midi-keyboard' : ''}`}
        ref={virtualKeyboard}
      >
        {keyboardKeys.map((key) => {
          return (
            <span
              ...
              className={`keyboard-note label ${key.color}-key ${
                keyNumbersPressed.includes(key.note) ? 'pressed' : ''
              }`}
            >
