...
export default function MIDIInput(props) {
  ...
  const rangeInputHandler = (note, velocity) => {
    let oscillatorToggleStepSize =
      127 / (effectsSettings.oscillatorWaveTypes.length - 1);
    ...
    switch (note) {
      ...
      case midiMapping.VCAGainInput:
        let normalizedVCAGain = normalize(
          velocity,
          127,
          effectsSettings.VCAGainMin,
          effectsSettings.VCAGainMax,
        );
      ...
      case midiMapping.LFOFrequencyInput:
        let normalizedLFOFrequency = normalize(
          velocity,
          127,
          effectsSettings.LFOFrequencyMin,
          effectsSettings.LFOFrequencyMax,
        );
        props.LFOFrequencyInputHandler(normalizedLFOFrequency);
        break;

      case midiMapping.LFOGainInput:
        let normalizedLFOGain = normalize(
          velocity,
          127,
          effectsSettings.LFOGainMin,
          effectsSettings.LFOGainMax,
        );
      ...
      case midiMapping.VCFQInput:
        let normalizedVCFQ = normalize(
          velocity,
          127,
          effectsSettings.VCFQMin,
          effectsSettings.VCFQMax,
        );
        props.VCFQInputHandler(normalizedVCFQ);
        break;

      case midiMapping.VCFFrequencyInput:
        let normalizedVCFFrequency = normalize(
          velocity,
          127,
          effectsSettings.VCFFrequencyMin,
          effectsSettings.VCFFrequencyMax,
        );
        props.VCFFrequencyInputHandler(normalizedVCFFrequency);
        break;

      case midiMapping.VCFGainInput:
        let normalizedVCFGain = normalize(
          velocity,
          127,
          effectsSettings.VCFGainMin,
          effectsSettings.VCFGainMax,
        );
        props.VCFGainInputHandler(normalizedVCFGain);
        break;

      case midiMapping.outputGainInput:
        let normalizedOutputGain = normalize(
          velocity,
          127,
          effectsSettings.outputGainMin,
          effectsSettings.outputGainMax,
        );
