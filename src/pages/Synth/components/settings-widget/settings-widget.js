  ...
  return (
    <div className="settings-widget">
      <div className="setting-header">
          ...
          <div className="setting-display-wrapper">
            ...
            <div className="setting-display">
              {Object.entries(props.notes).map(([note, i]) => {
                return (
                  <p key={note + i} className="setting-notes utility">
                    {getNameFromNoteNumber(note)}
                  </p>
                );
              })}
            </div>
        ...
        {props.VCFType ? (
          <div className="setting-display-wrapper">
            <p className="setting-display-title hide-mobile-landscape utility">
              Filter
            </p>
            <div className="type-toggle-wrapper text-toggle-wrapper">
              {effectsSettings.filterTypes.map((filterType) => {
                return (
                  <button
                    key={filterType}
                    onClick={() => props.setVCFType(filterType)}
                    className={`type-toggle utility ${
                      filterType == props.VCFType ? 'active' : 'inactive'
                    }`}
                  >
                    {filterType}
                  </button>
            ...
            <div className="type-toggle-wrapper">
              {effectsSettings.oscillatorWaveTypes.map((oscillatorType) => {
                return (
                  <button
                    key={oscillatorType}
                    onClick={() => props.setVCOType(oscillatorType)}
                    className={`type-toggle utility ${
                      oscillatorType == props.VCOWaveType
                        ? 'active'
                        : 'inactive'
                    }`}
                  >
                    <WaveIcons waveType={oscillatorType} />
                  </button>
                );
              })}
            </div>
            ...
            <div className="type-toggle-wrapper">
              {effectsSettings.oscillatorWaveTypes.map((oscillatorType) => {
                return (
                  <button
                    key={oscillatorType}
                    onClick={() => props.setLFOType(oscillatorType)}
                    className={`type-toggle utility ${
                      oscillatorType == props.LFOWaveType
                        ? 'active'
                        : 'inactive'
                    }`}
                  >
